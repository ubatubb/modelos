---
title: "Mortalidade infantil por pais"
...
```{r pacotes, include=FALSE,echo=FALSE,warning=FALSE}
#install.packages(c("ggplot2","dplyr","readr","kableExtra","gridExtra","readxl","rnaturalearth","rnaturalearthdata","sf", "summarytools","knitr","patchwork", "GGally", "rworldmap"))
```
...
## Existiam 195 países no banco de dados

Foram removidos alguns países da análise, entre eles: **Eswatini, Nauru, Sudão do Sul, Andorra, Mônaco, Vaticano, Palestina, Tuvalu**, pois não haviam informações sobre as variáveis estudadas
...
ggpairs(data = wor, columns = c(6,7,9))
...
ggplot(wor)+
  ...
  labs(title = "Quartís da mortalidade infantil por continente")+
...
&\quad + \beta_{3,j} I_{\text{Continente}_j} \hspace{4mm}, \text{para} \ j \in\{0,1,2,3,4\} \\ 
&\quad + \epsilon
\end{aligned}
$$
...
-   Não possui interpretação prática direta, pois a expectativa de vida e a quantidade de médicos nunca será $0$.
...
plot_ly(data = wor, 
        ...
        mode = 'markers',
        marker = list(size = 3)) %>%
  layout(title = "Expectativa de Vida, Log(Médicos) e Mortalidade Infantil",
         scene = list(
...
## Gráfico com os planos de regressão


```{r}

grid <- expand.grid(
  expect_vida = seq(min(wor$expect_vida, na.rm = TRUE), max(wor$expect_vida, na.rm = TRUE), length.out = 30),
  medic = seq(min(wor$medic, na.rm = TRUE), max(wor$medic, na.rm = TRUE), length.out = 30),
  cont = unique(wor$cont, na.rm = TRUE) # Cada continente será representado
)
wor <- wor[-179, ]

# Criando a grade de valores para o gráfico
grid <- expand.grid(
  expect_vida = seq(min(wor$expect_vida, na.rm = TRUE), max(wor$expect_vida, na.rm = TRUE), length.out = 30),
  medic = seq(min(wor$medic, na.rm = TRUE), max(wor$medic, na.rm = TRUE), length.out = 30),
  cont = unique(wor$cont) # Cada continente será representado
)

# Calculando o log de 'medic' e fazendo as previsões
grid$log_medic <- log(grid$medic + 0.1)
grid$pred <- predict(mod, newdata = grid)

# Plot com o plano de regressão
plot_ly(data = wor, 
        x = ~expect_vida, 
        y = ~log(medic + 0.1), 
        z = ~mort_inf, 
        color = ~cont,  # Aplicando a cor do continente
        colors = cores,  
        type = 'scatter3d', 
        mode = 'markers',  # Corrigido para 'mode = markers'
        marker = list(size = 3)  # Definindo o tamanho do marcador
) %>%
  add_trace(data = grid, 
            x = ~expect_vida, 
            y = ~log_medic, 
            z = ~pred, 
            type = 'mesh3d',  # Usando 'mesh3d' para representar o plano
            opacity = 0.5, 
            colorscale = "Viridis", 
            color = ~cont,  # Aplicando a cor do continente também no plano
...
Em um nível de 1% de significância, não há evidencias para rejeitar a hipótese nula, isto é, podemos dizer que os resíduos seguem uma distribuição normal
...
```
A autocorrelação dos resíduos não é estatisticamente significativa, isto é, a autocorrelação é proxima de 0, portanto, podemos assumir independencia dos resíduos
...
Austrália, Nova Zelandia, Kuwait, Chade e o Paquistão são outliers

## Teste de homocedasticidade dos resíduos

$$ 
H_0 : resíduos \rightarrow homocedástico \\
H_1 : resíduos \nrightarrow homocedástico
$$
```{r}
library(lmtest)
bptest(mod)
...
O pressuposto foi quebrado, o modelo pode não ser bom/suficiente
...
Vamos remover a Austrália, Nova Zelandia, Kuwait, Chade e o Paquistão, rodar o modelo novamente e fazer o teste de Breusch-Pagan


```{r}
wor_so <- wor[-c(25, 37,95,148, 121), ]
mod_so = lm(mort_inf ~ expect_vida + log(medic+0.1) + cont, data = wor_so)
bptest(mod_so)
```

O p-valor continua muito próximo de 0, portanto, não são os outliers que estão atrapalhando a homocedasticidade dos resíduos, mas o modelo é insuficiente para explicar a mortalidade infantil
...
Como os valores de $VIF_j$ estão bem abaixo de 10, podemos assumir que não há multicolinearidade

# Conclusão

## Conclusão

A análise indicou a presença de não $homocedasticidade$, evidenciada pela variação não constante dos resíduos ao longo dos valores preditos, o que compromete a precisão das estimativas. O $R^2$ ajustado foi moderado, sugerindo que o modelo explica uma fração significativa da variação na taxa de mortalidade infantil, mas ainda há variação não explicada. Isso reforça a necessidade de considerar outras variáveis ou utilizar um modelo mais sofisticado para melhorar o ajuste e a precisão das previsões.
